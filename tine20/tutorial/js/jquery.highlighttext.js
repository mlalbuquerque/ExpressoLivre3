...
(function($){
	...
	$.fn.highlightText = function () {
		...
		if ( arguments[2] && reStr ) {
			/*ALTERAÇÃO*/
	        //variável termo para guardar o termo 'limpo', sem caracteres de expressão regular
			termo = reStr;
		...
		if (!re) {
			/*ALTERAÇÃO*/
	        //o último argumento era "ig", o 'g' foi retirado para que a expressão não seja 'geral'
			//e aplique o glossário apenas na primeira ocorrência do termo
	    ...
	    return this.each( function() {
			...
			//alert($(this).parents().filter(".descricao_acao").html().indexOf(termo+'</a>')+" "+termo);
			...
			.each( function () {
				/*ALTERAÇÃO*/
	            //Condição para aplicar o glossário uma vez se o termo repetir
				if ($(this).parents().filter(".descricao_acao").html().indexOf(termo+'</a>') != -1) {
					return false;
				}
				var output;
	...
	$.fn.highlightText.defaultTagName = "a";
