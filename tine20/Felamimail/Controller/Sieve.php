...
class Felamimail_Controller_Sieve extends Tinebase_Controller_Abstract
{
    ...
    protected function _getServerSieveScript($_accountId)
    {
        ...
        $account = Felamimail_Controller_Account::getInstance()->get($_accountId)->user;
        
        foreach (array($this->_scriptName, $this->_oldScriptName, $account) as $scriptNameToFetch) {
            if (count($scripts) > 0 && array_key_exists($scriptNameToFetch, $scripts)) {
                ...
                if ($script) {
                    if($scriptNameToFetch == $account)
                         if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG))
                             Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Got USER SIEVE script for migration.');
                    if ($scriptNameToFetch == $this->_oldScriptName) {
                        if (Tinebase_Core::isLogLevel(Zend_Log::DEBUG)) 
                            Tinebase_Core::getLogger()->debug(__METHOD__ . '::' . __LINE__ . ' Got old SIEVE script for migration.');
                    }
                    if (Tinebase_Core::isLogLevel(Zend_Log::TRACE))
                        Tinebase_Core::getLogger()->trace(__METHOD__ . '::' . __LINE__ . ' Got SIEVE script: ' . $script);
